...
loC控制反转 Inverse of Control创建对象的权限，Java程序中需要用到的对象不再由程序员自己创建，而是交给loC容器来创建。
...
4. Servlet、Service、Dao
   ...
   当需求发生变更的时候，可能需要频繁修改Java代码。效率很低。
...
​	上述的方式并不能解决我们的问题，需求发生改变的时候我们仍然要修改java代码，如何在不修改java代码，就可以实现类的切换呢？
...
```properties
helloDao=com.tou.dao.impl.HelloDaoImpl
```
...
1. 强依赖 / 紧耦合 编译后无法修改，没有扩展性
2. 弱依赖 / 松耦合 编译后仍然可以修改，让程序须有更好的扩展性

自己放弃了创建对象的权限，将创建对象的方式教给了Bean，这种将控制权交给别人的思想，就是控制反转(IOC)

## 1.2 Spring IOC

装载bean的方式： XML 和 注解 xml用的很少。主流是注解
...
1. 扫描包的组件： 找到所有添加了注解的目标类
2. 获取原材料的组件： Class & BeanName -> BeanDefinitions（bean的集合）
3. 根据原材料动态创建Bean -> 装载到 IOC容器中（k-v）

### 手写代码思路

1. 自定义一个context， 构造器中传入要扫描的包
2. 获取这个包下所有的类
3. 遍历这些类，找出添加了@Components注解的类，活区它的Class和对应的beanName，封装成一个BeanDefinition存入集合Set，这个集合就是IoC自动装载的原材料
4. 遍历 Set 集合， 通过反射机制创建对象，同时检测属性有没有添加@Value注解，如果有，还需要给属性赋值，再将这些动态创建的对象以k-v的形式，存入到缓冲区。
5. 提供getBean 等方法，通过beanName取出对应的Bean即可
