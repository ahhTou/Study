...
- 初始标记：仅仅标记GC ROOTS 直接关联的对象，并且STW
- 并发标记：使用GC ROOTS TRACING算法，进行跟踪标记，无STW
- 重新标记：因为之前并发标记，其他用户线程不暂停，可能产生新的垃圾，所以重新标记并发标记过程中产生变动的对象，并且STW
...
这个引用计数法听起来不错，但是有一个弊端，如下图所示，循环引用时，两个对象的计数都为1，导致两个对象都无法被释放。![在这里插入图片描述](.\img\19.png)
...
第一步：
使用 jps 命令，查看程序的进程
![在这里插入图片描述](.\img\20.png)
第二步：
![在这里插入图片描述](.\img\20210209111229838.pngf)
...
- 不会有内存碎片
- 需要占用两倍内存空间
  ![在这里插入图片描述](.\img\28.png)

## 3、分代垃圾回收

![在这里插入图片描述](.\img\29.png)
...
![在这里插入图片描述](.\img\60.png)
![在这里插入图片描述](.\img\70.png)
